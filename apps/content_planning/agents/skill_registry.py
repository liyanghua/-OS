    ...
    def load_defaults(self) -> int:
        defaults = [
            ...
            SkillDefinition(
                ...
                skill_name="全链路一键策划",
                description="从机会卡到 production-ready 资产的全自动流程",
                trigger_keywords=["一键策划", "全链路", "自动策划"],
                agent_role="lead_agent",
                workflow_steps=["Brief编译", "模板匹配", "策略生成", "计划编排", "标题生成", "正文生成", "图片规划", "资产组包", "发布格式化", "质量评估"],
                executable_steps=[
                    SkillStep(tool_name="compile_brief", description="编译 Brief", arguments={"card": "$ctx.card"}),
                    SkillStep(tool_name="match_templates", description="模板匹配", arguments={"brief": "$ctx.brief"}),
                    ...
                    SkillStep(tool_name="generate_titles", description="标题生成", arguments={"plan": "$ctx.plan", "strategy": "$ctx.strategy"}),
                    SkillStep(tool_name="generate_body", description="正文生成", arguments={"plan": "$ctx.plan", "strategy": "$ctx.strategy"}),
                    ...
                    SkillStep(tool_name="format_for_publish", description="发布格式化", arguments={"bundle": "$ctx.asset_bundle", "strategy": "$ctx.strategy"}),
                    SkillStep(tool_name="evaluate_stage", description="质量评估", arguments={"stage": "asset", "opportunity_id": "$ctx.opportunity_id", "context": "$ctx.evaluation_context"}),
                ],
