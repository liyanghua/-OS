...
| 模式      | 说明                                                | 适用场景                              |
| --------- | --------------------------------------------------- | ------------------------------------- |
| 快照模式  | 把本机的 `data/` 全套打包，服务器解包即可           | 默认；客户现场无 LLM key、无登录态    |
...
| 路径                                        | 角色                                       |
| ------------------------------------------- | ------------------------------------------ |
| `scripts/export_dataset.sh`                 | 本机一键导出 `data/` + MediaCrawler jsonl  |
| `scripts/bootstrap_data.sh`                 | 服务器侧解包、权限、systemd 重启           |
...
3. 写入 `manifest.json`（导出时间、git commit、各表行数、JSON 大小）。
...
3. `--mode safe` 把已存在文件改名为 `*.bak.<ts>` 后再写入；
4. 调 `python -m apps.intel_hub.scripts.bootstrap_data sync-cards`
   把 `opportunity_cards.json` 写入 `xhs_review.sqlite`；
5. 调 `python -m apps.intel_hub.scripts.bootstrap_data validate`
   校验关键 SQLite 表均可连通；
6. `systemctl restart ontology-os`。
...
- 服务先 `systemctl stop ontology-os` 再 bootstrap，或加 `--restart-service` 让脚本接管。
